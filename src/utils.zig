...
pub const Compression = struct { // MARK: Compression
    ...
    pub fn zstdDeflate(allocator: NeverFailingAllocator, data: []const u8) []u8 {
        const bound = zstd.ZSTD_compressBound(data.len);

        var out = allocator.alloc(u8, bound);

        // std.debug.print("ZSTD compressing: {} bound: {}\n", .{ data.len, bound });

        const compressed_size = zstd.ZSTD_compress(out.ptr, bound, data.ptr, data.len, 10);

        // std.debug.print("ZSTD compressed: {}\n", .{compressed_size});

        if (zstd.ZSTD_isError(compressed_size) != 0) {
            const err_str = zstd.ZSTD_getErrorName(compressed_size);
            var buf: [128]u8 = undefined;
            const msg = std.fmt.bufPrint(&buf, "ZSTD compression failed: {s}", .{err_str}) catch "ZSTD compression failed";
            @panic(msg);
        }

        return allocator.realloc(out.ptr[0..bound], compressed_size);
        // return out[0..compressed_size];
    }
    ...
    pub fn zstdInflateTo(buf: []u8, data: []const u8) !usize {
        const decompressed_size = zstd.ZSTD_decompress(buf.ptr, buf.len, data.ptr, data.len);

        // std.debug.print("ZSTD decompressed : {}\n", .{decompressed_size});

        if (zstd.ZSTD_isError(decompressed_size) != 0) {
            const err_str = zstd.ZSTD_getErrorName(decompressed_size);
            var buf2: [128]u8 = undefined;
            const msg = std.fmt.bufPrint(&buf2, "ZSTD decompression failed: {s}", .{err_str}) catch "ZSTD decompression failed";
            @panic(msg);
        }
